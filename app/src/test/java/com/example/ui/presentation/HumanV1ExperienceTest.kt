...
class HumanV1ExperienceTest {
    ...
    @Test fun syncingStateIsNotSynced() = assertEquals("Syncing", syncPresentation(AuthState.Authenticated(profile), "Syncing", 0, null).title)
    ...
    @Test fun syncErrorKeepsLocalTruth() = assertTrue(syncPresentation(AuthState.Authenticated(profile), "Failed", 0, "network").detail.contains("locally"))
