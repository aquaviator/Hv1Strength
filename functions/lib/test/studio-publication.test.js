...
describe("governed Studio publication", function () {
    ...
    it("creates one new workout and plan version after one draft changes, then accepts an exact governed acknowledgement", async () => {
        ...
        for (const [id, override] of preserved)
            await root.collection("plannedWorkouts").doc(`plan-secure:${id}`).set({
        ...
        assert_1.strict.ok((await obsolete.get()).data()?.deletedAt != null);
        assert_1.strict.equal((await obsolete.get()).data()?.supersededByPlanVersionId, second.planVersionId);
        for (const [id] of preserved)
            assert_1.strict.equal((await root.collection("plannedWorkouts").doc(`plan-secure:${id}`).get()).data()?.deletedAt, null);
        assert_1.strict.equal((await unrelated.get()).data()?.deletedAt, null);
        ...
        const replay = await (0, studioPublication_1.publishStudioPlanForUid)(db, uid, { planId: "plan-secure", expectedRevision: 2, idempotencyKey: "second" });
        assert_1.strict.deepEqual({ ...replay, reusedPlan: second.reusedPlan }, second);
        assert_1.strict.deepEqual(await Promise.all(["publishedWorkouts", "publishedPlans", "plannedWorkouts"].map(name => root.collection(name).get().then(value => value.size))), countsBeforeReplay);
        assert_1.strict.deepEqual((await obsolete.get()).data(), obsoleteBeforeReplay);
