...
describe("Strength Firestore trusted identity rules", function () {
    ...
    it("allows the owner for every Strength synchronized collection", async () => {
        ...
        for (const collection of COLLECTIONS) {
            ...
            await (0, rules_unit_testing_1.assertSucceeds)((0, firestore_1.setDoc)((0, firestore_1.doc)(db, `users/${H1}/${collection}/${documentId}`), data));
            await (0, rules_unit_testing_1.assertSucceeds)((0, firestore_1.getDoc)((0, firestore_1.doc)(db, `users/${H1}/${collection}/${documentId}`)));
        }
    ...
    it("denies planner ownership forgery on create and update", async () => {
        ...
        for (const collection of ["trainingPlans", "plannedWorkouts"]) {
            ...
            const ref = (0, firestore_1.doc)(db, `users/${H1}/${collection}/${globalId}`);
            const valid = { globalId, humanUserId: H1, createdAt: 1, updatedAt: 1,
                deletedAt: null, revision: 1, originDeviceId: "synthetic-device" };
            await (0, rules_unit_testing_1.assertFails)((0, firestore_1.setDoc)(ref, { ...valid, humanUserId: H2 }));
            await (0, rules_unit_testing_1.assertSucceeds)((0, firestore_1.setDoc)(ref, valid));
            await (0, rules_unit_testing_1.assertFails)((0, firestore_1.setDoc)(ref, { ...valid, humanUserId: H2, revision: 2, updatedAt: 2 }));
        }
    });
    ...
    it("denies direct publication creates and reads only governed immutable versions", async () => {
        ...
        for (const [collectionName, contentType] of [
            ...
            await (0, rules_unit_testing_1.assertFails)((0, firestore_1.setDoc)(ref, value));
            await env.withSecurityRulesDisabled(async (context) => (0, firestore_1.setDoc)((0, firestore_1.doc)(context.firestore(), `users/${H1}/${collectionName}/${value.versionId}`), value));
    ...
    it("denies cross-owner publication access and owner reassignment", async () => {
        ...
        await env.withSecurityRulesDisabled(async (context) => (0, firestore_1.setDoc)((0, firestore_1.doc)(context.firestore(), path), value));
    ...
    it("allows only deterministic owner-bound immutable Human Strength delivery acknowledgements", async () => {
        ...
        await env.withSecurityRulesDisabled(async (context) => (0, firestore_1.setDoc)((0, firestore_1.doc)(context.firestore(), `users/${H1}/publishedWorkouts/${publication.versionId}`), publication));
        ...
        await (0, rules_unit_testing_1.assertFails)((0, firestore_1.setDoc)((0, firestore_1.doc)(owner, path), value));
        await env.withSecurityRulesDisabled(async (context) => (0, firestore_1.setDoc)((0, firestore_1.doc)(context.firestore(), path), { ...value, createdAt: new Date() }));
