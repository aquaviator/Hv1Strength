...
describe("atomic Studio plan-draft save", function() {
  ...
  it("atomically saves the eight-workout ten-week fixture and identical retry creates no duplicates", async () => {
    ...
    const first = await saveStudioPlanDraftForUid(db, uid, input); const before = await savedState();
    const second = await saveStudioPlanDraftForUid(db, uid, input); const after = await savedState();
  ...
  it("returns unchanged for a different key, dependency order and client timestamps", async () => {
    ...
    const legacyPlanRef = db.doc(`users/${owner}/planDrafts/atomic-plan`); const legacyPlan = (await legacyPlanRef.get()).data()!;
    legacyPlan.payload.weeks[0].placements.forEach((item: any) => item.dependency.expectedUpdatedAt = now);
    legacyPlan.contentChecksum = "legacy-request-envelope-checksum"; await legacyPlanRef.set(legacyPlan); const before = await savedState();
    const replay = request(8, 10, "different-key"); replay.create = false; replay.expectedRevision = 1;
    replay.dependencies.reverse(); replay.dependencies.forEach(item => item.expectedUpdatedAt = "2030-01-01T00:00:00.000Z");
    for (const week of (replay.plan as any).weeks) for (const placement of week.placements) placement.dependency.expectedUpdatedAt = "2030-01-01T00:00:00.000Z";
    const result = await saveStudioPlanDraftForUid(db, uid, replay);
    assert.equal(result.status, "UNCHANGED"); assert.equal(result.revision, 1); assert.deepEqual(await savedState(), before);
  });
  it("treats placement order and dependency revision changes as genuine edits", async () => {
    await seedWorkouts(); await saveStudioPlanDraftForUid(db, uid, request());
    const reordered = request(8, 10, "placement-order"); reordered.create = false; reordered.expectedRevision = 1;
    (reordered.plan as any).weeks[0].placements.reverse();
    const first = await saveStudioPlanDraftForUid(db, uid, reordered); assert.equal(first.revision, 2);
    await db.doc(`users/${owner}/workoutDrafts/workout-0`).update({ revision: 2 });
    const revision = request(8, 10, "dependency-revision"); revision.create = false; revision.expectedRevision = 2;
    (revision.plan as any).weeks[0].placements.reverse();
    revision.dependencies[0].expectedRevision = 2; (revision.plan as any).weeks[0].placements.find((item: any) => item.workoutId === "workout-0").dependency.expectedRevision = 2;
    const second = await saveStudioPlanDraftForUid(db, uid, revision); assert.equal(second.revision, 3);
    assert.equal((await savedState()).audits, 3);
  });
  it("updates once, removes obsolete dependencies and rejects stale updates", async () => {
    ...
    const result = await saveStudioPlanDraftForUid(db, uid, update); const beforeReplay = await savedState();
    const replay = await saveStudioPlanDraftForUid(db, uid, update); assert.equal(result.revision, 2); assert.equal(replay.idempotent, true);
    assert.deepEqual(await savedState(), beforeReplay);
    ...
    (stale.plan as any).title = "Stale conflicting edit";
  ...
  it("handles concurrent identical creates as one commit", async () => {
    await seedWorkouts(); const first = request(); const secondInput = request(8, 10, "concurrent-create-two");
    const [one, two] = await Promise.all([saveStudioPlanDraftForUid(db, uid, first), saveStudioPlanDraftForUid(db, uid, secondInput)]);
  ...
  it("handles concurrent identical updates as one revision and rejects a conflicting loser", async () => {
    await seedWorkouts(); await saveStudioPlanDraftForUid(db, uid, request());
    const one = request(7, 10, "concurrent-update-one"); one.create = false; one.expectedRevision = 1;
    const two = request(7, 10, "concurrent-update-two"); two.create = false; two.expectedRevision = 1;
    const results = await Promise.all([saveStudioPlanDraftForUid(db, uid, one), saveStudioPlanDraftForUid(db, uid, two)]);
    assert.deepEqual(results.map(item => item.revision), [2, 2]); assert.equal((await savedState()).audits, 2);
    const editA = request(6, 10, "conflict-a"); editA.create = false; editA.expectedRevision = 2;
    const editB = request(7, 10, "conflict-b"); editB.create = false; editB.expectedRevision = 2; (editB.plan as any).title = "Conflicting title";
    const settled = await Promise.allSettled([saveStudioPlanDraftForUid(db, uid, editA), saveStudioPlanDraftForUid(db, uid, editB)]);
    assert.equal(settled.filter(item => item.status === "fulfilled").length, 1);
    assert.equal(settled.filter(item => item.status === "rejected").length, 1);
    assert.equal((await savedState()).revision, 3); assert.equal((await savedState()).audits, 3);
  });
